
	<img class="center" src="/img/logo.png" />
	<div class="spacer"></div>


	<div class="span4 center well">
          	<!-- <div class="alert alert-error">
                <a class="close" data-dismiss="alert" href="#">×</a>Incorrect Username or Password!
            </div> -->
		<form method="POST" class="login" accept-charset="UTF-8">
			<input type="text" id="username" class="input-form" name="username" placeholder="Mail">
			<input type="password" id="password" class="input-form" name="password" placeholder="Password">
            <!-- <label class="checkbox">
            	<input type="checkbox" name="remember" value="1"> Remember Me
            </label> -->
			<button type="submit" id="submit" name="submit" class="btn btn-danger btn-large btn-block">Entra</button>
		</form>    
		<div class="text-center">Scarica il <a href="/info/intro">tutorial</a> oppure <a href="/index/signup">registrati</a></div>
	</div>
	<br><br><br><br>
			
