...
// 		$this->boxPrint($statOld);
	...
	public function getPercentuale ($idTarget, $workspace) {
		
		if ($workspace == 'area') {
			$this->loadModules('project');
			$projectModel = new Project();
			
			$actions = $projectModel->selectAllAreaAction($idTarget);
		}
		else if ($workspace == 'task') {
			$this->loadModules('action');
			$actionModel = new Action();
				
			$actions = $actionModel->selectActionByTask($idTarget);
		}
		...
		if (isset($nActionCompleted) && $nActionCompleted > 0) {
			$percentuale = $nActionCompleted/$nAction;
				
			if (is_int($percentuale)) {
				$percentuale = $percentuale.'0';
			}
			else {
				$n = explode('.', $percentuale);
				$percentuale = $n[1][0];
			}
		}
		else {
	...
	public function addMail ($id_receiver = null, $workspace, $index, $id_target, $id_sender) {
		
		switch ($workspace) {
			
			case 'action':
				$mail = $this->_addMailAction($id_receiver, $index, $id_target, $id_sender);
				break;
				
			case 'group':
				$mail = $this->_addMailGroup($id_receiver, $index, $id_target, $id_sender);
				break;
				
			case 'meeting':
				$mail = $this->_addMailMeeting($index, $id_target, $id_sender);
				break;
				
			case 'review':
				$mail = $this->_addMailReview($id_receiver, $index, $id_target, $id_sender);
				break;
					
			case 'update':
				$mail = $this->_addMailUpdate($id_receiver, $index, $id_target, $id_sender);
				break;
		}
		...
		if ($workspace != 'meeting') {
			...
			$insert = $projectModel->insertMail($mail);
		}
		else {
			$insert = $mail;
		}
	...
	public function _addMailAction ($id_receiver, $index, $id_target, $id_sender) {
		...
		$action = $actionModel->selectAreaByAction($id_target);
		$sender = $userModel->selectLoggedUser($id_sender);
		
		switch ($index) {
			
			case 1:
				$mail['text'] = $sender['nome'].' '.$sender['cognome'].' ti ha assegnato l\'action "'.$action['titolo'].'" ('.$action['nome'].' -> '.$action['titoloTask'].') ';
				break;
			
			case 2:
				$mail['text'] = $sender['nome'].' '.$sender['cognome'].' ha inserito una nuova action con titolo:"'.$action['titolo'].'" ('.$action['nome'].' -> '.$action['titoloTask'].') ';
			...
			case 3:
				...
				$mail['text'] = $sender['nome'].' '.$sender['cognome'].' ha cambiato lo stato di:"'.$action['titolo'].'" in '.$status.'! ('.$action['nome'].' -> '.$action['titoloTask'].') ';
				break;
				
			case 4:
				$mail['text'] = $sender['nome'].' '.$sender['cognome'].' ha completato l\'action"'.$action['titolo'].'" inserita da te. ('.$action['nome'].' -> '.$action['titoloTask'].') ';			
				break;
				
		}
		
		$mail['id_receiver'] = $id_receiver;
		$mail['id_sender'] = $id_sender;
		$mail['id_project'] = $action['id_project'];
		$mail['data'] = date('Y-m-d');
		
		return $mail;
	}
	
	public function _addMailGroup ($id_receiver, $index, $id_target, $id_sender) {
...
// 		index per i vari casi al momento solo uno quindi no switch
		$taskArea = $projectModel->selectAreaByTask($id_target);
		
		$this->loadModules('user');
		$userModel = new User();
		
// 		$this->boxPrint($taskArea);
// 		die;
		
		$user = $userModel->selectLoggedUser($id_sender);
		
		$mail['text'] = $user['nome'].' '.$user['cognome'].' ha inserito un nuovo Gruppo con nome:"'.$taskArea['titolo'].'" (Area: '.$taskArea['nome'].')';
		
		$mail['id_sender'] = $id_sender;
		$mail['id_receiver'] = $taskArea['id_responsabile'];
		$mail['id_project'] = $taskArea['project'];
		$mail['data'] = date('Y-m-d');
	...
	public function _addMailMeeting ($index, $id_target, $id_sender) {
		...
		$meeting = $projectModel->selectMeeting($id_target);
		$receivers = $userModel->selectUserProject($meeting['id_project']);
		$user = $userModel->selectLoggedUser($id_sender);
		
		$mail['data'] = date('Y-m-d');
		$mail['id_sender'] = $id_sender;
		$mail['id_project'] = $meeting['id_project'];
		$mail['text'] = $user['nome'].' '.$user['cognome'].' ha aggiunto un meeting dal titolo "'.$meeting['titolo'];
		
		foreach ($receivers as $receiver) {
			if ($receiver['id_user'] != $this->idLoggedUser) {
				$mail['id_receiver'] = $receiver['id_user'];
				$insert = $projectModel->insertMail($mail);
			}
		}
	...
	public function _addMailReview ($id_receiver, $index, $id_target, $id_sender) {
		...
		switch ($index) {
			...
			case 1:
				...
				$mail['id_project'] = $action['id_project'];
				$mail['text'] = $sender['nome'].' '.$sender['cognome'].' ti ha mandato in review l\'action "'.$action['titolo'].'" ('.$action['nome'].' -> '.$action['titoloTask'].') ';
				break;
			...
			case 2:
				...
				$elemento = $projectModel->selectListaFromElemento($id_target);
				
				$mail['id_project'] = $elemento['id_project'];
				$mail['text'] = $sender['nome'].' '.$sender['cognome'].' ti ha mandato in review il documento "'.$elemento['nome'].'" ('.$elemento['nome_lista'].' -> '.$elemento['nomeGruppo'].') ';
	...
	public function _addMailUpdate ($id_receiver, $index, $id_target, $id_sender) {
		...
		$mail['text'] = $sender['nome'].' '.$sender['cognome'].' ha fatto l\'update del documento "'.$elemento['nome'].'" ('.$elemento['nome_lista'].' -> '.$elemento['nomeGruppo'].') ';
